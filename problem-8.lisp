...
;; Finding the largest product of 5 consecutive digits in a 1000-digit number

(defconstant problem-8-digits
...
(defun digit-subsequences (l x)
  "Generate all consecutive subsequences of length X from the list L."
  (loop for i from 0 to (- (length l) x)
     collect (subseq l i (+ i x))))

(defun products-of-subsequences (l x)
  "Calculate a the products of all consecutive subsequences of length X
from the list L."
  (mapcar (lambda (x) (apply #'* x))
          (digit-subsequences l x)))

(defun largest (l) (apply #'max l))

(defun problem-8 (l x)
  "Calcuate the largest sum of a consecutive subsequence of length X
from the list L."
  (largest (products-of-subsequences l x)))
